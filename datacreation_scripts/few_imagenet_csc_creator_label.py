### create images text pair for few imagenet dataset
...
data_file = "datasets/csv/select_data/high_prob/top1_probability_per_class_VIT_B32.csv"
out = open(f"./datasets/csv/select_data/high_prob/caption_top1_probability_per_class_VIT_B32.csv", "w")
...
with open(data_file, "r") as f:
    ...
    for line in tqdm(lines[1:]):
        label, _, filepath = line.strip().split(",")
        class_name = openai_classnames[int(label)]

        for t in template:
            captions = t(class_name)
            out.write(f"{captions}\t{filepath.replace('./', '')}\t{label}\n")