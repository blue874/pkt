...
def main_worker(args):
    ...
    allclassname = []
    alllabel = []
    out = open(os.path.join(args.save_root, f"caption_random_seed{args.seed}_{args.n}shot_per_class.csv"), "w")
    out.write("title\tfilepath\tlabel\n")
    if args.dataset == "ImageNet":
        root = "./datasets/data/ILSVRC2012/train/"
        ...
        for i in range(len(list_folders)):
            ...
            #get random index from len(all_files)
            random_index = np.random.choice(len(all_files), args.n, replace=False)

            for j in random_index:
                allfile.append(os.path.join(path, all_files[j]))
                allclassname.append(class_name)
                alllabel.append(str(label))
                

    for i in range(len(allfile)):
        for t in template:
            caption = t(allclassname[i])
            out.write("%s\t%s\t%s\n" % (caption, allfile[i], alllabel[i]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n", type=int, default=1, help="number of sample per class")
    parser.add_argument("--dataset", type=str, default="ImageNet", help="dataset name")
    parser.add_argument("--data_location", type=str, default="./datasets/data", help="data location")
    parser.add_argument("--save_root", type=str, default="datasets/csv/select_data/caption", help="csv file")
