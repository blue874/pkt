...
class CLIPEncoder(torch.nn.Module):
    def __init__(self, args, keep_lang=False):
        super().__init__()
        try:
            if args.model.backbone == 'ViT-L-14':
                self.model, self.train_preprocess, self.val_preprocess = open_clip.create_model_and_transforms(
                    args.model.backbone, pretrained='laion400m_e31')
            elif args.model.backbone == 'ViT-B-16':
                print("****************Loading ViTB16 from openCLIP****************")
                self.model, self.train_preprocess, self.val_preprocess = open_clip.create_model_and_transforms(
                    args.model.backbone, pretrained='laion400m_e31')
            else:
                self.model, self.train_preprocess, self.val_preprocess = clip.load(
                    args.model.backbone, args.model.device, jit=False)
            self.cache_dir = args.model.cache_dir
        except:
...
class ClassificationHead(torch.nn.Linear):
    ...
    def load(cls, filename, logger=None):
        ...
        return utils.torch_load(cls, filename)
