...
ray.init(dashboard_port=34342+int(os.environ.get('LOCAL_RANK', 0)))
...
def collect_with_path_fn(batch):
    # 从batch中分离出不同类型的数据
    images = torch.stack([item[0] for item in batch])  # 将图像张量堆叠
    texts = torch.stack([item[1] for item in batch])   # 将文本张量堆叠
    labels = torch.tensor([item[2] for item in batch])  # 保持标签为列表
    image_paths = [item[3] for item in batch]          # 保持图像路径为字符串列表
    caption_texts = [item[4] for item in batch]        # 保持文本描述为字符串列表
    
    return images, texts, labels, image_paths, caption_texts
...
class CsvDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #image_path = os.path.join(hydra.utils.get_original_cwd(), image_path)#拼接工作目录和图片路径
        ...
        if self.return_label:
            ...
            if len(self.captions_list) > 0:
                ...
                return images, texts, label, image_path, caption_text

        if len(self.captions_list) > 0:
            return images, texts, texts_list

        return images, texts, image_path, caption_text
...
def get_wds_dataset(args, preprocess_img, is_train, epoch=0, floor=False, is_evaluate=False):
...
def get_csv_dataset(args, preprocess_fn, is_train, epoch=0, is_evaluate=False):
    ...
    sampler = None
    shuffle = is_train and sampler is None and not is_evaluate

    if is_evaluate:
        collect_fn = collect_with_path_fn
    else:
        collect_fn = None
...
def get_data(args, preprocess_fns, epoch=0, is_evaluate=False):
    preprocess_train, preprocess_val = preprocess_fns
    data = {}
    ...
    if not is_evaluate:
        ...
        data["train_evaluate"] = get_dataset_fn(args.ft_data,
                                        args.dataset_type)(args,
                                                            preprocess_val,
                                                            is_train=True,
                                                            epoch=epoch,
