...
class ImageFolderWithPaths(datasets.ImageFolder):
    def __init__(self, path, transform, flip_label_prob=0.0, classnames=None, templetes=None):
    ...
    def __getitem__(self, index):
        ...
        if self.classnames is None or self.templetes is None:
            return {
                'images': image,
                'labels': label,
            ...
            # ramdom select a template from a list of templates
            random_index = random.randint(0, len(self.templetes) - 1)
            selected_template = self.templetes[random_index]

            return {
                'images': image,
                'labels': label,
                'image_paths': self.samples[index][0],
                "captions": tokenize([selected_template(self.classnames[label])])[0]
            }
