...
class ImageNet:
    def __init__(
        ...
        self.custom = custom
        self.template = getattr(templates, templetes) if templetes is not None else None
        self.populate_train()
        self.populate_test()
    ...
    def populate_train(self):
        ...
        #traindir=os.path.join(self.location,'imagenet','images','train')
        self.train_dataset = ImageFolderWithPaths(traindir,
                                                  transform=self.preprocess, classnames=self.classnames, templetes=self.template)
    ...
    def get_test_path(self):
        ...
        #test_path= os.path.join(self.location,'imagenet','images','val_dirs')
        if not os.path.exists(test_path):
            test_path = os.path.join(self.location, 'ILSVRC2012', 'val')
            
            #test_path=test_path= os.path.join(self.location,'imagenet','images','val')
        return test_path
